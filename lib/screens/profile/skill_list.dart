...
class _SkillListState extends State<SkillList> {
  // Declare the variable as "late" to assign it later
  late List<Skill> availableSkills; 
  late Skill selectedSkill;
  //


  // Lifecycle method - needed to spin up the widget before it is available via the regular build method so that we can define our list for access at the class level before the build method is run below. Lesson 55 Outputting Vocation Skills.
  @override  
  void initState() {

    availableSkills = allSkills.where((skill) {
    return  skill.vocation == widget.character.vocation; 
    //the build method needs to be initialized to access the widget (WIDGET.character.vocation) which must be running to get access to the character model and assign a value from it to the variable/list.
  }).toList();
